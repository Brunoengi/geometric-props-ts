var GeometricProps = (function () {
    ...
    GeometricProps.prototype.calculateIx = function (_a) {
        var x0 = _a.x0, y0 = _a.y0, dx = _a.dx, dy = _a.dy;
        this._Ix += (x0 * (y0 * (dy + y0) + Math.pow(dy, 2) / 3)
            + dx * (y0 * (y0 / 2 + 2 * dy / 3) + Math.pow(dy, 2) / 4)) * dy;
    };
    ...
    GeometricProps.prototype.calculateIxy = function (_a) {
        var x0 = _a.x0, y0 = _a.y0, dx = _a.dx, dy = _a.dy;
        this._Ixy += (x0 * (x0 * (y0 + dy / 2) + dx * (y0 + 2 * dy / 3))
            + Math.pow(dx, 2) * (y0 / 3 + dy / 4)) * dy / 2;
    };
